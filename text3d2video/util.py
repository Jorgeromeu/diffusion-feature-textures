...
def project_vertices_to_features(
    ...
    rasterizer = MeshRasterizer(
        cameras=cam[batch_idx], raster_settings=raster_settings)
    fragments = rasterizer(mesh)

    # get visible faces
    visible_faces = rearrange(
        fragments.pix_to_face[0], 'h w 1 -> (h w)').unique()
    ...
    visible_points_ndc = cam[batch_idx].transform_points_ndc(
        visible_verts).cpu()
    ...
    vert_features = torch.zeros(mesh.num_verts_per_mesh()[
                                0], feature_dim).to(verts)
...
def sample_feature_map(
    ...
    out = F.grid_sample(batched_feature_map, grid,
...
def multiview_cameras(
    ...
    elevation = torch.linspace(
        start=0, end=end, steps=steps).repeat(steps) + add_angle_ele
