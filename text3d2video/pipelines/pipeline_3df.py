...
from diffusers.utils import (  # is_compiled_module,; randn_tensor,
...
from text3d2video.sd_feature_extraction import HookManager
...
class StableDiffusion3D2Video(
    ...
    def enable_model_cpu_offload(self, gpu_id=0):
        ...
        hook = None
        for cpu_offloaded_model in [self.text_encoder, self.unet]:
    ...
    def get_saved_self_attn(self):
        attn = []

        def get_attn(net):
            for name, subnet in net.named_children():
                if net.__class__.__name__ == "Attention":  # spatial Transformer layer
                    attn.append(net.saved_attn)
                    # net.requires_grad_(True)
                elif hasattr(net, "children"):
                    get_attn(subnet)

        for net_name, net in self.unet.named_children():
            get_attn(net)
        return attn
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # INJECT FEATURE
                # TODO: correct way of saving timestep, make sure to distinguish index, timestep, and correct order
                hook_manager = HookManager()

                if i == feature_timestep and inject_features:
                    module = self.unet.up_blocks[2]

                    def hook(module, inp, output):

                        feature_maps_stacked = torch.cat(
                            [feature_maps, feature_maps], dim=0
                        )

                        # output = output + feature_maps_stacked
                        print("features black")
                        output = output + feature_maps_stacked

                    hook_manager.add_named_hook("a", module, hook)
                ...
                pred_x0 = scheduler_out["pred_original_sample"]
                x_prev = scheduler_out["prev_sample"]
                ...
                if i == feature_timestep:
                    hook_manager.clear_named_hook("a")
        ...
        if return_intermediates:
            interm = [self.latent2image(img, return_type="np") for img in pred_x0_list]
            return (
